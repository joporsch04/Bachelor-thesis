...
To compare and verify the extended SFA rate from Chapter 2, the TDSE must be solved numerically without harsh approximations such as the SFA.
A reliable approach for comparison involves the use of the sampling method TIPTOE \cite{Park:18}, where the ionization yield from both methods is compared.
When expanding an existing model, such comparisons are particularly important to assess whether modifications align with expected outcomes.
...
Two methods were used, each with its own advantages and disadvantages.  
...
This can be viewed as the analytic continuation of a function into the complex plane.
...
Moreover, tRecX allows the gauge \eqref{eq:dipoleApprox} or \eqref{eq:dipoleApprox_velocity} in which the TDSE is solved to be specified, increasing its flexibility.
This feature will be important later.

\medskip
Throughout this thesis, tRecX was employed for two purposes.

First, it was used to solve the TDSE in the entire Hilbert space to obtain the coefficients $c_n(t)$, which are required for the extended SFA rate.
...
Second, tRecX served as a reference within the TIPTOE experiment to evaluate the performance of the extended SFA rate \eqref{eq:sfa_rate_improved}.
...
\bigskip
This method provides a way to compare ionization dynamics predicted by a numerical solver and by the extended SFA rate \eqref{eq:sfa_rate_improved}.
...
For better visualization of the underlying physics captured by TIPTOE, the background ionization $N_{\mathrm{F}}$ caused by $E_\mathrm{F}(t)$ is subtracted, and the ionization yield is normalized, so the formula reads
\begin{equation}
    \frac{N_{\mathrm{total}}-N_{\mathrm{F}}}{N_{\mathrm{max}}}=\frac{\delta N(\tau)}{N_{\mathrm{max}}}      \label{eq:tiptoeprop_normalized}
\end{equation}
However, interesting physics can be observed by comparing also the total ionization yield, as discussed in Section \ref{sec:diff_ion_SFA_tRecX}.

Typically, TIPTOE is not used for this kind of analysis but rather for its sampling capabilities.
Ionization rates are highly useful because TIPTOE enables sampling of the electric field of a laser pulse in the femtosecond to attosecond regime, which has broad applications in fields such as laser spectroscopy and medical physics.
