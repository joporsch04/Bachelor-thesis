...
\end{equation*}
With $\ket{p}$ being a plane wave. 
The formula for the wave function for hydrogen atom is well known:
\begin{equation}
...
Because of the structure of $\psi_{nlm}$ we can expect a result simular to \eqref{eq:psi_position_hydrogen}.
...
\end{equation*}
The final formula $\phi_{nlm}(\uvec{p})$ that can be also found in \cite{Bransdenatomsmolecules} in slightely different form, can then be expressed as:
\begin{align}
    \label{eq:phi_nlm}
