...
params = {'E_g': 0.5, 'αPol': 4.51, 'tau': 2.849306230484045, 'e1': 2.2807090369952894, 't0': 0.1, 't1': 3.043736601676354, 't2': 7.270940402611973, 'e2': 0, 't3': 0, 't4': 1, "div_p":2**-4, "div_theta":1, 'lam0': 850, 'intensity': 1e14, 'cep': 0}

delaycsv = pd.read_csv("/home/user/TIPTOE-Hydrogen/plot_ion_tau_calc_output_data/ion_prob_850nm_350nm_1.25e+14.csv")

delay = np.array(delaycsv["delay"].values) 
...
for file_name, lam0_pump, I_pump, lam0_probe, I_probe, FWHM_probe, cep_pump, cep_probe in file_params:
    if REDO_comp:
        ...
        ion_na_rate_SFA = IonRate(time_recon, laser_pulses, params, dT=0.5, kernel_type='exact_SFA', excitedStates=3, coeffType="numerical", gauge="length")
        ...
        for tau in delay:
            ...
            ion_na_SFA.append(1-np.exp(-(1-np.exp(-np.double(simpson(ion_na_rate_SFA, x=time_recon, axis=-1, even='simpson'))))))
            laser_pulses.reset()
            laser_pulses.add_pulse(lam0_probe, I_probe, cep_probe, FWHM_probe/AtomicUnits.fs, t0=-tau)
            ...
            laser_pulses.reset()
        output_file = f"/home/user/BachelorThesis/Bachelor-thesis/ionModel/python/dataOutput/ionProb_{file_name}.csv"
        writecsv_prob(output_file, delay, ion_qs, ion_na_GASFIR, ion_na_SFA, ion_na_reconstructed_GASFIR, ion_na_reconstructed_SFA)

    data_rate_delay = pd.read_csv(f"/home/user/BachelorThesis/Bachelor-thesis/ionModel/python/dataOutput/ionProb_{file_name}.csv")
